...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (path.startsWith('/userupdate/') && method === 'PATCH') {
      const email = decodeURIComponent(path.split('/userupdate/')[1]);
      const profile = body;
    ...
    if (path === '/subscription' && method === 'POST') {
      ...
      const subscription = {
        email, plan, tweetsPerMonth, price,
        tweetsUsed: 0,
        startDate: new Date(),
        endDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
      };
      await subscriptionCollection.updateOne({ email }, { $set: subscription }, { upsert: true });
    ...
    if (path === '/increment-tweet-count' && method === 'POST') {
      const { email } = body;
      await subscriptionCollection.updateOne({ email }, { $inc: { tweetsUsed: 1 } });
      return res.status(200).json({ success: true });
    }
    ...
    if (path === '/verify-payment' && method === 'POST') {
      const { razorpay_order_id, razorpay_payment_id, email, plan, tweetsPerMonth, price } = body;
      const subscription = {
        email, plan, tweetsPerMonth, price,
        tweetsUsed: 0,
    ...
    if (path === '/record-login' && method === 'POST') {
      ...
      const loginRecord = {
        email, browser, os, deviceType, ip, screenResolution, language,
    ...
    if (path.match(/^\/notification-settings\//) && method === 'PATCH') {
      const email = decodeURIComponent(path.split('/notification-settings/')[1]);
      const { notificationsEnabled } = body;
      await usercollection.updateOne({ email }, { $set: { notificationsEnabled } }, { upsert: true });
      return res.status(200).json({ success: true });
    }

    // GET NOTIFICATION SETTINGS
    if (path.match(/^\/notification-settings\//) && method === 'GET') {
      const email = decodeURIComponent(path.split('/notification-settings/')[1]);
